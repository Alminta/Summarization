...
def generateVarLen(seqNum,vocab,alpha,maxLen,seqLen,minSeqLen):
    ...
    for i in range(seqNum):
        ...
        i2=0
        numNums = 0 
        #tmp = int(minSeqLen+(seqLen-minSeqLen)*np.random.pareto(5,1))
        #rndLen = max(min(seqLen,tmp),minSeqLen)
        rndLen = np.random.randint(minSeqLen,seqLen)
...
def forward_pass(encoder, decoder, x, t, t_in, criterion, max_t_len, lenenc, lendec, teacher_forcing=True):
    ...
    #accuracy = (torch.max((pred == t), (t == 0))).type(torch.FloatTensor).mean() # Old broken acc. that counted padding as always correct instead of omitted.
    accuracy = torch.min(pred==t,t!=0).sum().type(torch.FloatTensor)/(t!=0).sum().type(torch.FloatTensor)
...
def forward_passNo(encoder, decoder, x, t, t_in, criterion, max_t_len, teacher_forcing):
    """
    Executes a forward pass through the whole model.
    :param encoder:
    :param decoder:
...
def trainNo(encoder, decoder, inputs, targets, targets_in, criterion, enc_optimizer, dec_optimizer, epoch, max_t_len):
    encoder.train()
    decoder.train()
    #batch_size = inputs.size(0)
    #enc_h = encoder.init_hidden(batch_size)
    ...
    for batch_idx, (x, t, t_in) in enumerate(zip(inputs, targets, targets_in)):
        #print(batch_idx)
        #print(x.size())    
        out, loss, accuracy = forward_passNo(encoder, decoder, x, t, t_in, criterion, max_t_len, True)
...
def testNo(encoder, decoder, inputs, targets, targets_in, criterion, max_t_len):
    encoder.eval()
    decoder.eval()
    with torch.no_grad():
        inputs = inputs.to(device)
        targets = targets.long().to(device)
        targets_in = targets_in.long().to(device)
        out, loss, accuracy = forward_passNo(encoder, decoder, inputs, targets, targets_in, criterion, max_t_len,
                                           teacher_forcing=TEACHER_FORCING)
    return out, loss, accuracy

def trainMo(encoder, decoder, inputs, targets, targets_in, criterion, enc_optimizer, dec_optimizer, epoch, max_t_len):
    encoder.train()
    decoder.train()
    #batch_size = inputs.size(0)
    #enc_h = encoder.init_hidden(batch_size)
    ...
    for batch_idx, (x, t, t_in) in enumerate(zip(inputs, targets, targets_in)):
        #print(batch_idx)
        #print(x.size())    
        out, loss, accuracy = forward_passMo(encoder, decoder, x, t, t_in, criterion, max_t_len, True)
...
def testMo(encoder, decoder, inputs, targets, targets_in, criterion, max_t_len):
    encoder.eval()
    decoder.eval()
    with torch.no_grad():
        inputs = inputs.to(device)
        targets = targets.long().to(device)
        targets_in = targets_in.long().to(device)
        out, loss, accuracy = forward_passMo(encoder, decoder, inputs, targets, targets_in, criterion, max_t_len,
                                           teacher_forcing=TEACHER_FORCING)
    return out, loss, accuracy


def forward_passMo(encoder, decoder, x, t, t_in, criterion, max_t_len, teacher_forcing):
    """
    Executes a forward pass through the whole model.
    :param encoder:
    :param decoder:
